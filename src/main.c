...
main() {
	...
	err = bind(listenfd, (struct sockaddr_in *)&server_addr, sizeof(server_addr));
	if (err < 0) {
		RM_LOG_PERR("Bind of server's port to managing socket failed");
		exit(EXIT_FAILURE);
	}
	...
	while(rm.state != RM_CORE_ST_SHUT_DOWN) {
		...
		if (read_n == -1) {
			...
			switch (read_n) {
				case EAGAIN:
					RM_LOG_ERR("Nonblocking I/O requested on TCP control socket");
				case EINTR:
					RM_LOG_ERR("TCP control socket: interrupted");
					continue;
				case EBADF:
					RM_LOG_ERR("TCP control socket passed is not a valid descriptor or nor open for reading");
				case EFAULT:
					RM_LOG_ERR("TCP control socket: buffer is outside accessible address space");
				case EINVAL:
					RM_LOG_ERR("TCP control socket: unsuitable for reading or wrong buffer len");
				case EIO:
					RM_LOG_ERR("TCP control socket: I/O error");
				case EISDIR:
					RM_LOG_ERR("TCP control socket: socket descriptor refers to directory");
				default:
...
//				break;
			...
			default:
				RM_LOG_ERR("Unknown TCP message type, this can't happen");
				exit(EXIT_FAILURE);
		}
