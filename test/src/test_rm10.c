...
test_rm_cmd_1(void **state) {
    ...
    for (; i < RM_TEST_FNAMES_N; ++i) {
        ...
        RM_LOG_INFO("PASSED test #1 (commandline utility - local push, default block size): files [%s] [%s] passed delta reconstruction, files are the same", buf_x_name, f_y_name);
    }
...
test_rm_cmd_2(void **state) {
    ...
    for (; i < RM_TEST_FNAMES_N; ++i) {
        ...
        if (f_y != NULL) fclose(f_y);
        f_y = NULL;
        RM_LOG_INFO("PASSED test #2 (commandline utility - local push): files [%s] [%s] passed delta reconstruction for all block sizes, files are the same", buf_x_name, f_y_name);
    }
